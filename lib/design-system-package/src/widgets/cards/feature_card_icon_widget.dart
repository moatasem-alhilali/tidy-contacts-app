...
class FeatureCardIconWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final cardPrimaryColor = primaryColor ?? context.colors.brandColor;
    ...
    final iconSize = math.min(minDimension * 0.20, 30.sp).clamp(24.sp, 30.sp);

    // Responsive text size
    final textSize = math.min(minDimension * 0.10, 14.sp).clamp(10.sp, 16.sp);
  ...
  Widget _buildContent(
    ...
    if (!hasTitle) {
      // Icon only - center it
      return Center(child: _buildIconContainer(iconSize, minDimension));
    }
    ...
    if (needsCompactLayout) {
      ...
      return Row(
        ...
        children: [
          ...
          Flexible(flex: 3, child: _buildTitleText(context, textSize * 0.9)),
        ],
      );
    }
  ...
  Widget _buildIconContainer(double iconSize, double minDimension) {
    return Container(
      ...
      child: IconTheme(
        data: IconThemeData(color: iconColor ?? Colors.white, size: iconSize),
